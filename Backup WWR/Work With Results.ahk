...
Menu, Tray, Icon, Imagens\wwr.ico
;CONFERIR HORARIO E VER SE PROGRAMA PODE ABRIR;
WinHttp := ComObjCreate("WinHttp.WinHttpRequest.5.1")
WinHttp.Open("GET", "http://worldtimeapi.org/api/ip", false) ; Get date from the internet
try
{
	WinHttp.Send() ; tentando executar
}
catch ; if there is an error, this block will execute
{
	msgbox,, Sem conexão com a internet, Seu computador não tem uma conexão com a internet. Fechando programa.
	ExitApp
}
...
ExpirationDate := 20210820 ; data de vencimento ANO/MES/DIA

if (CurrentDate >= ExpirationDate or !data) ; comparar as duas datas
{
	msgbox,1, Fim do período de testes, O período de testes acabou. Você gostaria de continuar usando o aplicativo?
    IfMsgBox,Ok
    Run, http://wa.me/5532991233902
    ExitApp
    Return
}
else
...
;JANELA PRINCIPAL
Gui, 1: Show, w500 h500, Work With Results - Assistente para Marketplaces
Gui, 1: Add, Picture, x0 y0 w500 h500, Imagens\Background.png
Gui, 1: +MinSize500x500
Gui, 1: Add, Button, x22 y80 w110 h30 gResumo, Resumo
Gui, 1: Add, Button, x22 y140 w110 h30 gAnuncios, Meus Anuncios
Gui, 1: Add, Button, x22 y200 w110 h30 gPerguntas, Perguntas
Gui, 1: Add, Button, x22 y260 w110 h30 gVendas, Minhas Vendas
Gui, 1: Add, Button, x22 y320 w110 h30 gMercadoPago, Mercado Pago
Gui, 1: Add, Button, x368 y380 w110 h30 gcontato, Contato desenvolvedor
Gui, 1: Add, Button, x22 y380 w110 h30 gSuporte, Suporte Mercado Livre
Gui, 1: Add, Button, x368 y80 w110 h30 gAnunciarSemelhante, Anunciar Semelhante
Gui, 1: Add, Button, x368 y140 w110 h30 gCalculadora, Calculadora de anuncios
Return

;GRUPOS DOS BOTÕES DA JANELA PRINCIPAL
calculadora:
Run, https://docs.google.com/spreadsheets/d/1YyIX7k4DNqn8RyV0oTOJB6WG8e3LcQuku00x1IsWwdM/edit#gid=1803134083
return
suporte:
Run, https://www.mercadolivre.com.br/ajuda/content/696/contact/338?item=MLB1337118168
return

contato:
Gui, 2: Show, w300 h300, Contato do desenvolvedor
Gui, 2: Add, Picture, x0 y0 w300 h300, Imagens\contatodev.png
Gui, 2: +Owner
Gui, 2: +MinSize300x300
Gui, 2: Add, Button, x95 y260 w110 h30 gwhatsapp, Contato WhatsApp
Return
...
resumo:
Run, http://mercadolivre.com.br/resumo
return

anuncios:
Run, https://www.mercadolivre.com.br/anuncios/lista?filters=ACTIVE
return

vendas:
Run, https://www.mercadolivre.com.br/vendas/lista
return

perguntas:
Run, https://www.mercadolivre.com.br/perguntas/comprador#menu-user
return

mercadopago:
Run,http://mercadopago.com.br
return

anunciarsemelhante:
MsgBox, 1, Anunciar Semelhante, Vá para o anúncio que deseja replicar e então, clique em OK.
IfMsgBox,Cancel
Return

Else
;SOLICITANDO NOVO TÍTULO
InputBox, UserInput, Work With Results, Insira o título do anúncio para começar
Title1 = %UserInput%

Sleep, 1000

;Clicando em Vender um igual
Text:="|<Vender um>*190$71."

if ok:=FindText(1498,271,150000,150000,0,0,Text)
{
  CoordMode, Mouse
...
WinWait, Anunciar
Sleep, 1500
;CLICANDO EM NÃO O TENHO AGORA
Text:="|<Não o tenho agora>*207$68.Dzk000000007zy00000000300k00000000k0A00000000A0300000000300k1XB00020k0A0MnU000UA0307AwQ1kQz00k1PF9Ua2Nk0A0Gnq8MUbw0304RxW689z00k17F9Ua2Nk0A0Enlk70nw03000000001zzU00000000Dzk000000008"

if ok:=FindText(1009,691,150000,150000,0,0,Text)
{
  CoordMode, Mouse
  X:=ok.1, Y:=ok.2, W:=ok.3, H:=ok.4, Comment:=ok.5
  MouseMove, X+W//2, Y+H//2
...
;ENCONTRANDO BOTÃO CONTINUAR
Text:="|<Continuar>*179$67.zzzzzzzzzzzzzzzzwzzzzzzUTzzaTzzzzzX7zznzzzzzzXz3UE61aMADnzAnAnAnAWDtzb9aNaNbtDwzbYnAnAn0byDtmNaNaNCHzX4nAnAnAW9zs73aM6NUM4zzzzzzzzzzzzzzzzzzzzzzzk"

if ok:=FindText(1260,898,150000,150000,0,0,Text)
{
  CoordMode, Mouse
  X:=ok.1, Y:=ok.2, W:=ok.3, H:=ok.4, Comment:=ok.5
  MouseMove, X+W//2, Y+H//2
...
;ENCONTRANDO BOTÃO CONFIRMAR
Text:="|<Confirmar>*178$66.zzzzzzzzzzzzzzzzzzzzzzzzzzztzzzzzy1zzy1zzzzzsszzyTzzzzztzUkA920633tz4FaN6MaF7lzCHaNCNbtDtzCHaNCNa1DtzCHaNCNYtDss4HaNCNYFDy1UnaNCNa1DzzzzzzzzzzzzzzzzzzzzzzU"

if ok:=FindText(1260,800,150000,150000,0,0,Text)
{
  CoordMode, Mouse
  X:=ok.1, Y:=ok.2, W:=ok.3, H:=ok.4, Comment:=ok.5
  MouseMove, X+W//2, Y+H//2
...
;ENCONTRANDO A BARRA DE DIGITAÇÃO
Loop{
	ImageSearch, x, y, 0, 0, A_ScreenWidth, A_ScreenHeight, Imagens/titleline.png
	If (ErrorLevel = 0){
		x+=5
		y-=3
		Click, %x%, %y%
...
;INSERINDO O TEXTO SOLICITADO ANTERIORMENTE
Send %Title1%
Sleep, 1000

;CLICANDO EM CONTINUAR APÓS INSERIR TÍTULO
Text:="|<Continuar>*178$67.zzzzzzzzzzzzzzzzzzzzzzzzzzzzzzzzzzzzzzzDzzzzzs7zztbzzzzzslzzwzzzzzzszkM41UNa33wznAHAnAn8XyTtmNaNaNyHzDttAnAnAk9zXyQaNaNaHYzslAnAnAn8WTy1kNa1aM61DzzzzzzzzzzzzzzzzzzzzzzzzzzzzzzzzzzzzzzzzzzzzzzzzzzzzzzzzU"

if ok:=FindText(1260,387,150000,150000,0,0,Text)
{
  CoordMode, Mouse
  X:=ok.1, Y:=ok.2, W:=ok.3, H:=ok.4, Comment:=ok.5
  MouseMove, X+W//2, Y+H//2
...
Text:="|<Seguinte>*178$68.zzzzzzzzzzzzzzzzzzzzzzzzzzzzzzzzzzzzzzzzzzzzzz0zzzzzDzzzzU7zzzznzszzslzzzzzzyDzyDw7UNnA11UzUy8l6Qn6Mlbw1bAtbAnaAszkE0CNnAtX0DzU03aQnCMk3ssbstbAnaAzy0Mn4MXAtX6TkD0s70nCQM7zzzztzzzzzzzzzzATzzzzzzzzzkDzzzzzzzzzzzzzzzzzzzzzzzzzzzzzzzzzzzzzzzzU"

if ok:=FindText(1291,904,150000,150000,0,0,Text)
{
  CoordMode, Mouse
  X:=ok.1, Y:=ok.2, W:=ok.3, H:=ok.4, Comment:=ok.5
  MouseMove, X+W//2, Y+H//2
...
;CLICANDO EM GARANTIA? PARA TIRAR SELEÇÃO DA CAIXA

Text:="|<Barra de titulos>*169$71.00000000000000000000000000000000k0D00000000VU0z000000010017RXsPDVjDaDU6z4MyFXzTAFUM60Nk3768M330A0n02AAEk260Mza7wMMVbwA0n3AAMkl3AM01a6MMFVX6ME0zAQklX37AlVVqDtUz66CMz30A00000000007k0000000000D00000000000000000000000000000000001"

if ok:=FindText(562,451,150000,150000,0,0,Text)
{
  CoordMode, Mouse
  X:=ok.1, Y:=ok.2, W:=ok.3, H:=ok.4, Comment:=ok.5
  MouseMove, X+W//2, Y+H//2
...
;CLICANDO EM ANUNCIAR
Text:="|<Barra de titulos>*179$69.zzzzzzzzzzzzzzzzzzzzzzzzzzzzzzzzzzzzzzzzzzzzzzzXzzzzzznzzzsDzzzzzyTzzz1zzzzzzzzzztD0tn0Q2M63y8sXCMX6nAkTn7CNnCFyTWDwQtnCNmDn0FzU3CNnCFyF2Dw0NnCNmDmAFz7lCMXCMqFWDsy9n0NnUH0Fzzzzzzzzzzzzzzzzzzzzzzzzzzzzzzzzzzzzzzzzzzzzzzw"

if ok:=FindText(1074,903,150000,150000,0,0,Text)
{
  CoordMode, Mouse
  X:=ok.1, Y:=ok.2, W:=ok.3, H:=ok.4, Comment:=ok.5
  MouseMove, X+W//2, Y+H//2
...
Text:="|<Ver Anuncio>*177$71.zzzzzzzzzzzzzzzzzzzzzzzzzzzzzzzDzzbtszzzzzwzzzDnnzzzzzzzzzzb463kM6QUQ4k6F4TYF4t4F95Ab9zwb9mQbmSF0Hy1CHYtDYy6TbtmQb9mT9wAPDl4t4HYGFMw2Tk9n0bA4kzzzzzzzzzzzzzzzzzzzzzzzzzzzzzzzzzzzw"

if ok:=FindText(689,523,150000,150000,0,0,Text)
{
  CoordMode, Mouse
  X:=ok.1, Y:=ok.2, W:=ok.3, H:=ok.4, Comment:=ok.5
  MouseMove, X+W//2, Y+H//2
